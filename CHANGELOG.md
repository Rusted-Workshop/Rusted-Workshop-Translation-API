...
- Source-hash based dedup for `POST /v1/tasks`: every upload is fingerprinted with
  SHA-256 of the raw `.rwmod` bytes; re-submitting the same file with the same
  `target_language` and `translate_style` returns the existing `task_id` (with
  `X-Task-Reused: true` header and `reused: true` in the body) instead of
  starting a duplicate translation. A new `source_hash` column + composite
